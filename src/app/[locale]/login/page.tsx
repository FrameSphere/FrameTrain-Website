  ...
  return (
    <div className="space-y-3 mb-6">
      ...
      >
        ...
        <span className="font-medium text-sm">{t('googleLogin')}</span>
      </a>
      ...
      >
        ...
        <span className="font-medium text-sm">{t('githubLogin')}</span>
      </a>
...
function LoginContent() {
  ...
  useEffect(() => {
    ...
    if (oauthError) {
      const hasMessage = t.has(`errors.${oauthError}`)
      if (hasMessage) setError(t(`errors.${oauthError}` as any))
    }
  }, [searchParams, t])
        ...
        <div className="w-full max-w-md">
          <Suspense fallback={
            <div className="glass-strong rounded-2xl shadow-2xl p-8 border border-white/10 flex items-center justify-center min-h-[400px]">
              <div className="text-gray-400">{t('loadingFallback')}</div>
            </div>
          }>
            <LoginContent />
          </Suspense>
